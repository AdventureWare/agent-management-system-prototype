<script lang="ts">
	...
	import TaskQueueControls from './TaskQueueControls.svelte';
	...
	function isClosedTask(task: Pick<TaskRow, 'status'>) {
	...
	function buildTaskQueueCollection(
		...
		function summarizeTaskBranch(task: TaskRow): TaskQueueBranchState {
			...
			const summary = {
				...
				branchAllDone:
					isClosedTask(task) && childSummaries.every((childSummary) => childSummary.branchAllDone)
	...
	let queuePresetCounts = $derived.by(() => ({
		...
		needsAttention: tasks.filter((task) => !isClosedTask(task) && taskNeedsAttention(task)).length,
		completed: tasks.filter((task) => isClosedTask(task)).length
	}));
		...
		{:else}
			<div class="space-y-4 sm:space-y-5">
				<TaskQueueControls
					...
					bind:selectedReadinessLevel
					bind:selectedAutonomyLevel
					bind:selectedRiskLevel
					bind:selectedBlockedFilter
					bind:selectedDelegationMode
					bind:selectedTaskView
					bind:selectedSortField
					bind:selectedSortDirection
					bind:selectedQueueFocus
					statusOptions={data.statusOptions}
					{workflowFilterOptions}
					delegationModeOptions={DELEGATION_MODE_OPTIONS}
					{queuePresetCounts}
					{activeQueuePresetId}
					{activeTaskRowCount}
				...
				>
					...
					{@render taskTable(
						selectedTaskView === 'completed' ? 'Completed tasks' : 'Open tasks',
						'',
						visibleTaskRows,
						selectedTaskView === 'completed'
