<script lang="ts">
	...
	let filteredProviders = $derived(data.providers.filter((provider) => matchesProvider(provider, query)));
</script>
...
<section class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-6 py-8">
	<div class="flex flex-col gap-3">
		...
		<h1 class="text-3xl font-semibold tracking-tight text-white">Browse provider setups before editing</h1>
		<p class="max-w-3xl text-sm text-slate-300">
			Providers should be easy to scan by service, auth mode, readiness, and worker usage. This page
			now acts as an index. Open a single provider when you need to change credentials, defaults, or
			operational notes.
		</p>
	</div>
	...
	<div class="grid gap-4 md:grid-cols-4">
		<article class="card border border-slate-800 bg-slate-950/70 p-5">
			...
			<p class="mt-2 text-sm text-slate-400">Saved provider profiles across local, cloud, and API access.</p>
		</article>
		...
		<article class="card border border-slate-800 bg-slate-950/70 p-5">
			...
			<p class="mt-2 text-sm text-slate-400">Profiles marked ready instead of planned or pending setup.</p>
		</article>
		...
		<article class="card border border-slate-800 bg-slate-950/70 p-5">
			...
			<p class="mt-2 text-sm text-slate-400">Workers currently attached to saved provider records.</p>
		</article>
	</div>
	...
	<section class="card border border-slate-800 bg-slate-950/70 p-6">
		<div class="flex flex-col gap-2">
			...
			<p class="text-sm text-slate-400">
				These are good first shapes for the system: one local coding surface, one chat surface, and
	...
	<div class="grid gap-6 xl:grid-cols-[0.88fr_1.12fr]">
		...
		>
			...
			<div class="flex flex-wrap items-center justify-between gap-3">
				<p class="text-xs text-slate-500">
					Local CLI setups work well for Codex. OAuth and API-key modes keep room for future providers.
				</p>
							...
							>
								...
								<div class="mt-4 grid gap-3 sm:grid-cols-3">
									...
									<div class="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
										...
										<p class="mt-2 text-sm text-white">{provider.defaultModel || 'Unset'}</p>
									</div>
									<div class="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
										<p class="text-[11px] tracking-[0.16em] text-slate-500 uppercase">Launcher</p>
										<p class="mt-2 text-sm text-white">{provider.launcher || 'None'}</p>
									</div>
								</div>
								...
								>
									<span>{provider.baseUrl || 'No base URL override'}</span>
