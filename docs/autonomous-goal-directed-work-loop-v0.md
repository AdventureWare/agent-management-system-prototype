...
| Prompt and work-packet generation        | Usable as a bounded packet surface; prompt/context stuffing has been reduced.     | `src/lib/workflow-prompts.ts` builds planner, executor, research, and reviewer prompts from selected task/project/goal/run/review fields; executor, research, and reviewer paths use bounded task-scoped project context while planner prompts can retain broader project memory. `src/lib/server/goal-work-packets.ts` and `src/lib/server/agent-work-packets.ts` expose `work-packet get_agent_work_packet` as structured packet fields plus rendered prompt through `/api/agent-work-packets/get_agent_work_packet`, CLI, and MCP. `src/lib/server/task-threads.ts` records launch-context readback through the existing `Run.contextSummary` field, and `TaskLoopReport.latestRun` exposes that context through CLI/API/MCP. Further reduction should be evidence-driven rather than treated as the default next slice.                                                                                                                                                                                       |
...
- Manifest-backed goal-loop read commands exist for `list_active_goals`, `get_goal_context`, `get_goal_progress`, `get_goal_success_criteria`, `get_goal_blockers`, `get_actionable_work`, `get_blocked_work`, `get_awaiting_review`, `get_next_recommended_action`, `explain_task_eligibility`, and `get_task_loop_report`.
...
- Manifest-backed run-result recording, progress preview, reviewed progress apply, and guarded task transitions exist as `run-result record_run_result`, `record_validation_result`, `record_blocker`, `record_followup_recommendations`, `create_followup_task`, `request_review_from_run`, `request_approval_from_run`, `mark_task_blocked_from_run`, `preview_progress_updates`, and `apply_progress_updates`, backed by `src/lib/server/agent-run-results.ts`.
...
- Production read-loop coherence is covered by `npm run test:agent-work-loop-smoke`, which now starts from `goal-loop get_operator_console` and sequences recommendation, task-loop report, work packet, run-result evidence, guarded review request, guarded approval request, accepted/completed goal progress, workflow/thread association readback, latest-run launch-context readback, and readback using in-memory fixture state.
- Live read-side probing against `data/app.sqlite` found and fixed an anti-stall bug: a `running` Goal with only closed linked work now routes to continuation planning instead of `goal_complete`. Token-authenticated CLI/API probing then verified `manifest --resource goal-loop` and `goal-loop get_operator_console` against the running AMS long-term Goal through the real bearer-token path.
- Managed-run context probing verified `goal-loop get_operator_console` can be called with only `AMS_AGENT_RUN_ID`; the CLI resolves current context, passes resolved project/goal/task scope, and the operator console uses the Goal-level continuation path when the selected current task is terminal.
- Materializable continuation paths now include an explicit policy: preview `goal-loop materialize_suggested_task` with `validateOnly=true`, create the draft task only through an explicit command, and do not auto-launch it.
- Validate-only materialization probing verified the explicit policy through the managed-run CLI path: with only `AMS_AGENT_RUN_ID` set, `goal-loop materialize_suggested_task --json '{"validateOnly":true}'` resolved project/goal context and returned a proposed `in_draft` planning task without changing runtime task state.
- The previewed planning task was then explicitly materialized through the same managed-run CLI/API path. The created runtime task is `task_ea2e9503-3683-4d9b-986f-c18255fd0e76`; task-loop readback classifies it as `needs_planning`, points to `work-packet:get_agent_work_packet --task task_ea2e9503-3683-4d9b-986f-c18255fd0e76`, and direct task-scoped operator-console and work-packet readback now resolve its own project/Goal correctly. Existing task planning paths return `continuationPolicy.mode=read_only`.
- The materialized planning task then produced `docs/autonomous-work-loop-v0-5-next-task-plan.md`, attached that artifact through `intent prepare_task_for_review`, opened review `review_0b8ba260-01b8-4ab1-894d-105da26d9ab2`, and was approved as `done`.
- The continuation assessment task `task_5d58d051-4512-46a9-8752-959dc969781e` identified task-only scope drift as the next concrete gap; it was approved as `done`.
- The regression task `task_ec8a0d72-5832-4830-b8bd-d4a591bbf235` added cross-surface task-only scope coverage and was approved as `done`.
- The closeout assessment task `task_e230f3bd-9735-4a40-8656-7736cedc0b3f` was approved as `done`, closing v0.5 as complete enough for stop-and-review.

The main remaining gap is no longer operator-path consolidation or task-only
readback. The next intentional milestone should decide whether to add a narrow
managed continuation-runner proof around existing explicit commands. Prompt/
context stuffing has been reduced through task-scoped prompt context,
launch-context readback summaries, task-loop latest-run context readback, and
manifest closeout playbooks. Automatic acceptance of work remains intentionally
unimplemented.
...
Status: Completed for the agent-facing packet API and strengthened for launch-context readback. `work-packet get_agent_work_packet` returns structured packet fields plus a rendered prompt through CLI/API/MCP; managed launches now record structured readback commands in `Run.contextSummary`, and task-loop reports expose latest-run prompt/context evidence. Further prompt reduction should be driven by concrete packet or launch evidence.
...
Status: Partially completed and ready for a stop-and-review checkpoint. Goal detail exposes goal-loop task classification, next action, and the shared operator path. Task detail exposes the task-loop report and the same operator path. Governance queue items carry the shared operator path for review, approval, and escalation cards. Run detail exposes run-result preview, project/goal progress preview proposals, and captured launch-context readback. Agent-facing CLI/API/MCP exposes goal-loop reads, `goal-loop get_operator_console`, per-task loop reports with latest-run prompt/context readback, work packets, run-result recording, progress previews, reviewed progress apply, guarded review and approval requests from run evidence, blocked-task updates from run evidence, and follow-up task creation. Manifest discovery now includes closeout-first run-result guidance and a `close_out_run_result` playbook. Remaining integration should be driven by runtime evidence instead of more dashboard surface area.
...
Autonomous Work Loop v0.5 is closed as complete enough for stop-and-review.
The v0.5 closeout assessment in
`docs/autonomous-work-loop-v0-5-closeout-assessment.md` was approved through
review `review_0eaf2323-9b07-4159-8720-5e0903f7a15e`, closing
`task_e230f3bd-9735-4a40-8656-7736cedc0b3f` as `done`.

The broader AMS long-term Goal remains `running`, and the goal-level operator
console will still offer generic planning fallback when no open scoped work
exists. Do not treat that fallback as more v0.5 work. The next implementation
milestone should be selected intentionally.

The strongest candidate is `Autonomous Work Loop v0.6: Managed continuation
runner proof`: a narrow runner contract that wraps existing explicit AMS
commands with validate-only previews, task-loop readback, and hard stops at
review, approval, blocker, clarification, unsafe, or missing-access gates.

If implementation continues immediately, target one concrete remaining gap:

1. Expose additional guarded state transitions only where domain helpers can validate them and return readback, keeping automatic acceptance out of v0.
2. Add continuation-runner behavior only if it is a narrow wrapper around the existing explicit `goal-loop:materialize_suggested_task` command, with validation/readback and no broad autonomous scheduler.
3. Continue prompt/context reduction only if concrete packet or launch evidence shows the loop is still carrying unnecessary context.

The prior ready-to-paste prompts for reviewed progress application and operator-path consolidation are complete. New task prompts should target one of the remaining evidence-backed gaps above.
