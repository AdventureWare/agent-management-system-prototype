...
| ----------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Goals                               | Partially usable                        | `Goal` and `GOAL_STATUS_OPTIONS` live in `src/lib/types/control-plane.ts`. App/API routes exist at `src/routes/app/goals`, `src/routes/app/goals/[goalId]`, `src/routes/api/goals`, and `src/routes/api/goals/[goalId]`. Agent list/get/create/update support exists in `src/lib/server/agent-control-plane-api.ts`, `scripts/ams-cli.mjs`, and `/api/agent-capabilities`. Goal-scoped classification exists in `src/lib/server/goal-work-loop.ts`; goal detail uses it through `src/routes/app/goals/[goalId]/+page.server.ts` and `+page.svelte`.                                                                                                                            | Agents can list and inspect goals today through CLI/API/MCP. The missing v0 agent affordance is an explicit `get_goal_context`, `get_goal_progress`, and `get_goal_success_criteria` surface that wraps goal, project, task, run, review, and blocker evidence. |
...
| Tasks/subtasks                      | Usable by agents now                    | `Task` includes goal/workflow/template links, parent task links, dependency IDs, delegation packet fields, execution contract fields, status, risk/autonomy/readiness/rigor, review/approval metadata, run/thread links, blockers, and attachments in `src/lib/types/control-plane.ts`. Task create/update/list/get and attachment operations are exposed through `src/routes/api/tasks`, `src/lib/server/agent-control-plane-api.ts`, `scripts/ams-cli.mjs`, and the MCP plugin. Subtask/delegation support exists in `src/routes/api/tasks/[taskId]/decompose`, `src/lib/server/task-decomposition-action.ts`, and `src/lib/server/task-child-handoffs.ts`. | The raw operations exist. The gap is a domain-level `create_followup_task`, `link_task_to_goal`, and `link_task_dependency` interface that prevents duplicate work and records why the task exists.                                                             |
| Task status                         | Partially usable                        | Stored statuses are `in_draft`, `ready`, `in_progress`, `review`, `blocked`, and `done` in `src/lib/types/control-plane.ts`. Derived readiness modes are computed by `src/lib/server/delegation-readiness.ts`. Goal-loop classifications such as `actionable_now`, `awaiting_review`, `needs_planning`, and `approval_required` are computed in `src/lib/server/goal-work-loop.ts`.                                                                                                                                                                                                                                                                                            | Stored lifecycle is adequate. Agents need structured derived classifications and explanations instead of inventing status semantics from prompt text.                                                                                                           |
| Blockers/dependencies               | Partially usable                        | Tasks have `blockedReason` and `dependencyTaskIds`. Helpers such as `taskHasUnmetDependencies`, `getOpenReviewForTask`, and `getPendingApprovalForTask` live in `src/lib/server/control-plane.ts`. Goal-loop helpers explain unmet dependencies in `src/lib/server/goal-work-loop.ts`.                                                                                                                                                                                                                                                                                                                                                 | Agents can set `blockedReason` and dependency IDs, but blocker type, open question, missing access, and research-vs-clarification are still mostly inferred. v0 should expose `get_goal_blockers`, `mark_task_blocked`, and `explain_task_eligibility`.         |
...
| Prompt/work-packet generation       | Partially usable                        | Mode-specific prompts live in `src/lib/workflow-prompts.ts`. Goal-loop work packets live in `src/lib/server/goal-work-packets.ts`. Managed-run prompts are built by `src/lib/server/task-threads.ts`. Task execution UI displays packet details in `src/lib/components/tasks/TaskExecutionPanel.svelte`.                                                                                                                                                                                                                                                                                                                                                                       | Work packets are now more selective than a single generic prompt. The gap is that they still carry too much operational boilerplate and do not expose all packet pieces as structured state/tool outputs.                                                       |
| Existing APIs/routes/server actions | Usable by agents now / partially usable | Agent-facing capability discovery is in `src/routes/api/agent-capabilities/+server.ts` and `src/lib/server/agent-capability-manifest.ts`. Current context is in `src/routes/api/agent-context/current/+server.ts` and `src/lib/server/agent-current-context.ts`. Intent wrappers are in `src/routes/api/agent-intents/[intent]/+server.ts` and `src/lib/server/agent-intent-actions.ts`. Project/goal/task/thread routes are listed in `docs/ams-cli-reference.md`.                                                                                                                                                                                                            | The foundational API exists. The next v0 work should normalize goal-loop read operations and run-result recording into the same manifest-backed API/MCP registry.                                                                                               |
...
| Completion result instructions                                                         | Ask the agent to state changes, validation, follow-ups, and whether durable state should change.                                                                                              | Run-result recording API and review/task update services              | Keep short completion instructions temporarily, but add `record_run_result`, `record_validation_result`, `record_blocker`, and `record_followup_recommendations` tools so the result becomes durable state.                                                                                         |
...
3. MCP server exposes safe AMS operations to external agents/Codex.
   ...
   Existing plugin: `plugins/ams-control-plane`. It already exposes manifest-backed task, goal, project, governance, session, and thread tools. v0 should add read-only goal-loop and packet tools before adding new mutation tools.
...
| `create_followup_task`   | Create a linked task from a run result, blocker, or review. | Source `taskId`/`runId`, title, objective, relation reason.              | Draft/framed follow-up task linked to source goal/project. | Mutation; dedupe against existing titles/source IDs.                                         | Task create; run `followUpTaskIds`; goal-run preview.                      | v0 as draft-only       |
...
| `record_run_result`               | Record structured completion/partial/failure result. | `runId`, summary, result, changed files/artifacts, classification. | Updated run plus previewed state transitions.             | Mutation; should not auto-accept unless authorized.                      | `goal-run-result-preview.ts`, `Run` fields.                   | v0 as record-only                           |
| `record_validation_result`        | Record validation commands and outcome.              | `runId`, commands, pass/fail, output summary.                      | Updated run validation fields.                            | Mutation; keep output summaries bounded.                                 | `Run.validationSummary`, run detail.                          | v0                                          |
| `record_blocker`                  | Record blocker found during a run.                   | `runId`, `taskId`, blocker text/type.                              | Updated run/task blocker.                                 | Mutation; should set task blocked only when requested or policy permits. | `Run.blockersFound`, `Task.blockedReason`.                    | v0                                          |
| `record_followup_recommendations` | Save proposed follow-ups without creating tasks yet. | `runId`, recommendations.                                          | Run follow-up recommendation data or draft task previews. | Mutation; draft-only unless user/tool creates tasks separately.          | `Run.followUpTaskIds` exists; recommendation storage is thin. | later                                       |
...
- Read-only goal/project/task/run/review context.
- Goal-loop read tools: active goals, goal context, goal progress, blockers, actionable work, awaiting review, next recommended action, task eligibility.
- Work-packet reads: selective project/task/goal/run packet output with rendered prompt as one field.
- Safe mutations already supported by domain logic: create/update task, link task/goal/dependencies, request review, request approval, attach artifact, create draft follow-up task, record run result, record validation result, record blocker.
...
1. Extend `src/lib/server/agent-capability-commands.js` and `src/lib/server/agent-capability-manifest.ts` with read-only goal-loop and work-packet capabilities.
2. Add API routes or agent-intent actions backed by `src/lib/server/goal-work-loop.ts` and `src/lib/server/goal-work-packets.ts`.
3. Regenerate or extend plugin tool definitions in `plugins/ams-control-plane`.
4. Add MCP/manifest tests next to `src/lib/server/ams-control-plane-mcp.spec.ts` and `src/lib/server/agent-capability-manifest.spec.ts`.
5. Add mutation tools only after read-only behavior is stable, using existing task/governance/run helpers.
...
The single best next implementation task is: Normalize goal-loop read operations into the agent API.
...
Task: Implement read-only agent-facing goal-loop APIs.

Use the existing Goal, Project, Task, Run, Review, Approval, readiness, and goal-work-loop helpers. Do not create a new milestone abstraction and do not create duplicate task/workflow/skill/planning systems.

Implement a minimal read-only agent API surface for:
...
- src/lib/server/delegation-readiness.ts
- src/lib/server/agent-control-plane-api.ts
- src/lib/server/agent-capability-manifest.ts
- src/routes/api/agent-capabilities/+server.ts
- scripts/ams-cli.mjs
- plugins/ams-control-plane/README.md

Prefer one coherent API/manifest pattern that can later back MCP tools. Return structured JSON with source IDs, classifications, reasons, safety/permission notes, and suggested readback commands. Keep all operations read-only. Add focused tests for actionable work, blocked work, awaiting review, approval-required work, and eligibility explanation.

Validation: run the relevant unit tests for the new helper/API/manifest coverage and report the commands and results.
```
