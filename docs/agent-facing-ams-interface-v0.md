...
| Completion result instructions                                                         | Ask the agent to state changes, validation, follow-ups, and whether durable state should change.                                                                                              | Run-result recording API and review/task update services              | Keep short completion instructions temporarily, but prefer `record_run_result`, `record_validation_result`, `record_blocker`, `record_followup_recommendations`, `create_followup_task`, `request_review_from_run`, `request_approval_from_run`, and `mark_task_blocked_from_run` so result evidence becomes durable state before task/review/approval mutations. |
...
| `get_task_loop_report`        | Return one task's current control-loop readback.                 | `taskId`; optional `goalId` and `projectId`.                     | Task, project, goal, classification, latest run, review, approval, blockers, dependencies, artifacts, decisions, work-packet pointer, and next action. | Read-only; no schema or state mutation.                                                       | `src/lib/server/task-loop-report.ts`, `goal-work-loop.ts`, `goal-work-packets.ts`.                 | v0     |
| `materialize_suggested_task`  | Create or link the current fallback draft task.                  | `goalId` or `projectId`; optional `validateOnly`.                | Created or deduped draft task plus task-loop and next-action readback guidance.                                                                        | Mutation; explicit only, validate-only supported, no launch or automatic acceptance.          | `src/lib/server/agent-goal-loop-actions.ts`; `/api/agent-goal-loop/materialize_suggested_task`.    | v0     |
...
| `request_approval_from_run`       | Open task approval from completed run evidence.          | `runId`, optional mode, summary, optional `validateOnly`.           | Preview or pending approval/task readback.            | Guarded mutation; no approval, rejection, or acceptance.                                                  | `run-result request_approval_from_run`.                                   | v0                                          |
...
Status: Implemented. Manifest readback lists `goal-loop list_active_goals`, `get_goal_context`, `get_goal_progress`, `get_goal_success_criteria`, `get_goal_blockers`, `get_actionable_work`, `get_blocked_work`, `get_awaiting_review`, `get_next_recommended_action`, `explain_task_eligibility`, and `get_task_loop_report`.
